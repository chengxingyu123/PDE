...
public class PluginContentPage extends ContentPage {
	...
	private void createPluginClassGroup(Composite container) {
		...
		boolean forTransformBundle = fMainPage.isForTransformBundle();
		fGenerateClass.setSelection(forTransformBundle ? true : ((settings != null) ? !settings.getBoolean(S_GENERATE_ACTIVATOR) : true));
		gd = new GridData();
		fGenerateClass.setEnabled(!forTransformBundle);
		gd.horizontalSpan = 2;
		...
		fUIPlugin.setSelection(forTransformBundle ? false : ((settings != null) ? !settings.getBoolean(S_UI_PLUGIN) : true));
		gd = new GridData();
		fUIPlugin.setEnabled(!forTransformBundle);
		gd.horizontalSpan = 2;
	...
	public void updateData() {
		...
		data.setDoGenerateClass(fGenerateClass.getSelection()); // Chris: I removed the enabled check from your patch - this will be fixed as true for transform bundles.
	...
	public void setVisible(boolean visible) {
		if (visible) {
			...
			boolean forTransformBundle = fMainPage.isForTransformBundle();
			fGenerateClass.setEnabled(!forTransformBundle && !fData.isSimple());
